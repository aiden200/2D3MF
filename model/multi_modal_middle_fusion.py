...
class EatConvBlock(nn.Module):
    def __init__(self, out_dim):
        ...
        self.adaptive_pool = nn.AdaptiveAvgPool1d(out_dim)

    def forward(self, x):
        x = x.permute(0, 2, 1) # Changing (B, 512, 768) to (B, 768, 512)
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.adaptive_pool(x) # Adjusting the size directly to (B, 768, 10)
        x = x.permute(0, 2, 1) # Changing back to (B, out_dim, 768)
        return x


def calculate_conv1d_output_size(input_length, kernel_size, stride=1, padding=0):
    output_length = (input_length - kernel_size + 2 * padding) // stride + 1
    return output_length

# input_length = 512  # Example input length
# kernel_size = 3     # Example kernel size
# stride = 2          # Example stride
# padding = 1         # Example padding

# output_length = calculate_conv1d_output_size(input_length, kernel_size, stride, padding)
